...
export default function ReceiptPage() {
    ...
    const handleSaveReceipt = async () => {
        if (receiptRef.current) {
            try {
                const dataUrl = await domToPng(receiptRef.current, { scale: 2, backgroundColor: "#ffffff" });
    ...
    return (
        <div className="min-h-screen bg-stone-100 p-6 flex flex-col items-center justify-center">
            ...
            <div ref={receiptRef} className="bg-white w-full max-w-md p-6 rounded-3xl shadow-xl space-y-6 relative overflow-hidden mb-6 border border-stone-100">
                <div className="absolute top-0 left-0 w-full h-3 bg-primary"></div>
                
                <div className="text-center space-y-1">
                    <div className="flex justify-center mb-2"><img src={logo} alt="Logo" className="h-20" /></div>
                    <h1 className="text-2xl font-bold text-stone-800">Amber Coffee</h1>
                    <p className="text-stone-400 text-xs uppercase tracking-widest">Official Receipt</p>
                </div>
                ...
                <div className="bg-stone-50 p-4 rounded-xl flex justify-between items-center border border-stone-100">
                    ...
                    <div className="text-right">
                        ...
                        <p className="text-xs font-bold text-stone-600">{data.timestamp ? data.timestamp.split(",")[0] : new Date().toLocaleDateString()}</p>
                        <p className="text-xs font-bold text-stone-600">{data.timestamp ? data.timestamp.split(",")[1] : new Date().toLocaleTimeString()}</p>
                    </div>
                </div>
                            ...
                            <div className="flex gap-2">
                                ...
                                <div>
                                    <span className="text-stone-600 block">
                                        {item.name} {item.addon && <span className="text-[10px] bg-orange-100 text-orange-700 px-1 rounded font-bold">+{item.addon}</span>}
                                    </span>
            ...
            {/* Actions */}
             <div className="w-full max-w-md space-y-3">
                ...
                <Link to="/" className="block w-full text-center text-stone-500 py-3 text-sm font-bold hover:text-stone-800">Back to Home</Link>
            </div>
        </div>
    );
}